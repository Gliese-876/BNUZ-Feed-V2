...
`npm run snapshot` 不再只执行单轮抓取；脚本会保留已成功结果，并对 `partial`、`timeout`、`network_error`、`parser_error`、`empty` 状态的站点按源增量重试，直到全部恢复或达到最大尝试次数。
...
| `BNUZ_FEED_SNAPSHOT_RETRY_DELAY_MS` | `1500` | 两轮失败源补抓之间的等待时间，单位毫秒 |

### 消息源复核约定

- 复核快照缺漏时，只把持续产出时间序列内容的动态栏目视为“消息源”。
- 明显静态栏目，如规章、办事指南、设备中心、楼宇分布、图片/风采展示等，不因为当前无条目而判定为漏抓。
- 对可疑动态栏目，先比对“配置频道”和“快照实际频道”，再用实时页面 DOM 与 parser 逐页复核。
- 当前已确认并修复两类真实漏抓：`国际交流与合作办公室 / 新闻速递` 与 `北京师范大学知行书院 / 新闻快讯`。
